...
while true; do
    # Проверяем статус оптимизации перед отрисовкой меню
    STATE=$(uci -q get firewall.@defaults[0].drop_invalid)
    if [ "$STATE" = "1" ]; then
        OPT_STATUS="${C_GREEN}[ВКЛ]${C_NONE}"
    else
        OPT_STATUS="${C_RED}[ВЫКЛ]${C_NONE}"
    fi

    clear
    echo -e "${C_CYAN}==================================================${C_NONE}"
    echo -e "${C_GREEN}       УТИЛИТЫ ДЛЯ OPENWRT И ПОДКОПА (v1.3)       ${C_NONE}"
    echo -e "${C_CYAN}==================================================${C_NONE}"
    ...
    echo -e " 6) ⚙️  Оптимизация сети $OPT_STATUS"
    echo -e " 0) ❌ Выход"
    echo "--------------------------------------------------"
    ...
    case "$choice" in
        ...
        5)
            ...
            ;;
        6)
            sh <(wget -qO- "$BASE_URL/net_optimizer.sh")
            ;;
        0)
            echo -e "\nВыход. Удачи!\n"
            exit 0
    ...
    echo -e "\n${C_CYAN}Нажмите Enter, чтобы вернуться в меню...${C_NONE}"
    read dummy
done